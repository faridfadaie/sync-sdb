...
module.exports = function(config){
	...
	this.sync = function(req, res){
		...
		if (req.method == "POST"){//this should create a new model
			//created_at, collection_name, updated_at, owner, read_access, write_access parameters are treated differently and are reserved words.
			...
			self.sdb.putItem(domain, id, params, function(err){
				...
				delete params['write_access'];
				delete params['read_access'];
				res.json(params);
			});
		}
		...
		if (req.method == "GET"){//this should return models (or model)
			if (requested_id){//the client is asking for only one model (not the whole collection)
				...
				var query = "select * from "+domain+" where collection_name='" + collection_name + "' and (read_access in ('*', '"+req.user._id+"') or owner='"+req.user._id+"')";
