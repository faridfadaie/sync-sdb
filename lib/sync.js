	...
	this.sync = function(req, res) {
		...
		if (req.method == "POST") { //this should create a new model
			...
			function create_object(parent) {
				...
				params['write_access'] = params['write_access'] || self.config.permissions.default_write;
				params['read_access'] = params['read_access'] || self.config.permissions.default_read;
